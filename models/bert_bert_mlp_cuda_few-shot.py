...
def few_shot_training(model, train_dataloader, savedir_fold):
    ...
    while True:
        ...
        dev_loss, dev_labels, dev_preds = evaluate_bert(model, train_dataloader)
        ...
        if macro_f1 > best_f1:
            ...
            # Save model
            #print("Saving model...")
            torch.save(model.state_dict(), savedir_fold)
            best_f1 = macro_f1
            best_loss = dev_loss
            patience = 0
        else:
        ...
        training_str = "epoch {} loss {} dev_loss {} dev_macro_f1 {}".format(
            epoch,
...
def main():
    ...
    parser.add_argument('--template_num', type=int, default=1, required=True)
    parser.add_argument('--template_id', type=int, default=0)
    parser.add_argument('--soft_template', default=False, action='store_true')
    parser.add_argument('--template_len', type=int, default=10)
    ...
    if args.soft_template:
        ...
        args.from_fold = os.path.join(args.savedir, "output/checkpoint_{}_{}_{}.pt".format(
            'multi' if args.multi_template else 'single',
            args.template_num if args.multi_template else args.template_id,
            args.protocol))
        args.savedir_fold = os.path.join(args.savedir, "output/checkpoint_few_shot_{}_{}_{}.pt".format(
            'multi' if args.multi_template else 'single',
            args.template_num if args.multi_template else args.template_id,
            args.protocol))
    ...
    # few-shot (one shot)
    few_shot_train_excerpt = random.sample(range(0, X_test_data.shape[0]), args.few_shot)
    few_shot_test_excerpt = [i for i in range(1, X_test_data.shape[0]) if i not in few_shot_train_excerpt]

    X_few_shot_train_data = X_test_data[few_shot_train_excerpt]
    y_few_shot_train = y_test[few_shot_train_excerpt]
    x_len_few_shot_train = x_len_test[few_shot_train_excerpt]
    x_few_shot_train_feats = X_test_feats[few_shot_train_excerpt]
    x_chunk_len_few_shot_train = x_chunk_len_test[few_shot_train_excerpt]

    X_few_shot_test_data = X_test_data[few_shot_test_excerpt]
    y_few_shot_test = y_test[few_shot_test_excerpt]
    x_len_few_shot_test = x_len_test[few_shot_test_excerpt]
    x_few_shot_test_feats = X_test_feats[few_shot_test_excerpt]
    x_chunk_len_few_shot_test = x_chunk_len_test[few_shot_test_excerpt]
    ...
    few_shot_train_dataset = data_utils.ChunkDataset(X_few_shot_train_data, x_few_shot_train_feats, x_len_few_shot_train, x_chunk_len_few_shot_train, y_few_shot_train)
    few_shot_test_dataset = data_utils.ChunkDataset(X_few_shot_test_data, x_few_shot_test_feats, x_len_few_shot_test, x_chunk_len_few_shot_test, y_few_shot_test)
    ...
    few_shot_train_dataloader = torch.utils.data.DataLoader(few_shot_train_dataset, batch_size=args.batch_size, shuffle=False)
    few_shot_test_dataloader = torch.utils.data.DataLoader(few_shot_test_dataset, batch_size=args.batch_size, shuffle=False)
    ...
    model = BERT_BERT_MLP(args.bert_model,
                          ...
                          use_features=args.features, bert_freeze=10, templates=templates, is_soft=args.soft_template, template_len=args.template_len)
    model.cuda()
    ...
    if args.do_eval:
        # Load model
        model.load_state_dict(torch.load(args.from_fold, map_location=lambda storage, loc: storage))

        few_shot_training(model, few_shot_train_dataloader, args.savedir_fold)

        model.load_state_dict(torch.load(args.savedir_fold, map_location=lambda storage, loc: storage))

        y_test, y_pred = evaluate_sequences(model, few_shot_test_dataloader)
